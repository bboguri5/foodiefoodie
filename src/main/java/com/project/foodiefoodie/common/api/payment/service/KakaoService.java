...
//        log.info("totalPrice KakaoService - {}", totalPrice instanceof Integer);
        ...
        try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(connection.getOutputStream()))) {
            ...
            queryParam.append("&quantity=" + totalQuantity) // 총 주문 수량
                    ...
                    .append("&approval_url=" + "https://foodiefoodie.co.kr/success-order") // 결제 성공시 redirect_url 지정
                    .append("&cancel_url=" + "https://foodiefoodie.co.kr/cancel-order") // 결제 취소시 redirect_url 지정
                    .append("&fail_url=" + "https://foodiefoodie.co.kr/fail-order"); // 결제 실패시 redirect_url 지정
